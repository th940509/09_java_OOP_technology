...
// Thread 클래스를 상속 받아서 쓰레드 기능을 구현할 수 있다.
class Music extends Thread {
	
	boolean isPlayMusic = true;
	
	// Thread 클래스로부터 제공되는 run() 메서드 이며 동시적으로 실행할 명령어를 기술하면 된다.
	@Override
	public void run() {
...
// Runnable 인터페이스를 구형하여 쓰레드 기능을 구현 할 수 있다.
...
public class OOPEx12 {
	...
	public static void main(String[] args) {
		...
		// Thread클래스를 상속받을 경우 객체에서 start()메서드를 이용하여 Thread를 시작한다 -> 클래스 안에 오버라이딩된 run()메서드 호출
		music.start();
